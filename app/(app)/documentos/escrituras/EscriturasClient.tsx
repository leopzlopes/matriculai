...
import type { TipoEscritura, DadosDocumento, DocumentoGerado, Parte, DadosImovel } from '@/lib/documentos/types';
import { LABEL_ESCRITURA } from '@/lib/documentos/types';

interface InitialData {
  imovel?: Partial<DadosImovel>;
  outorgante?: Partial<Parte>;
}

interface EscriturasClientProps {
  initialData?: InitialData;
}

const TIPOS = Object.entries(LABEL_ESCRITURA) as [TipoEscritura, string][];
...
export function EscriturasClient({ initialData }: EscriturasClientProps) {
  const [tipo, setTipo] = useState<TipoEscritura | ''>('');
  const [outorgante, setOutorgante] = useState<Parte>({
    ...emptyParte(),
    ...(initialData?.outorgante ?? {}),
  });
  const [outorgado, setOutorgado] = useState<Parte>(emptyParte());
  const [imovel, setImovel] = useState<DadosImovel>({
    ...emptyImovel(),
    ...(initialData?.imovel ?? {}),
  });
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-xs text-slate-400 text-center italic">
          Minuta gerada por IA — revisar com advogado e tabelião antes da lavratura
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <SectionTitle>Tipo de Escritura</SectionTitle>
      ...
      {tipo && (
        <>
          {/* Outorgante */}
          <div>
            <SectionTitle>Outorgante (Vendedor / Doador)</SectionTitle>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className={inputClass()} value={outorgante.qualificacao ?? ''} onChange={(e) => setOutorgante({ ...outorgante, qualificacao: e.target.value })} placeholder="Ex: advogado, empresário..." />
              </div>
          ...
          {/* Outorgado */}
          <div>
            <SectionTitle>Outorgado (Comprador / Donatário)</SectionTitle>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className={labelClass()}>Nome completo *</label>
              ...
              <div>
                <label className={labelClass()}>Qualificação profissional</label>
                <input className={inputClass()} value={outorgado.qualificacao ?? ''} onChange={(e) => setOutorgado({ ...outorgado, qualificacao: e.target.value })} placeholder="Ex: advogado, empresário..." />
              </div>
              <div className="md:col-span-2">
                <label className={labelClass()}>Endereço completo *</label>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="md:col-span-2">
                <label className={labelClass()}>Descrição registral *</label>
                <textarea
                  className={`${inputClass()} resize-none`}
                  rows={3}
                  value={imovel.descricao}
                  onChange={(e) => setImovel({ ...imovel, descricao: e.target.value })}
                  placeholder="Descrição conforme consta na matrícula do imóvel..."
                />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input className={inputClass()} value={valor} onChange={(e) => setValor(e.target.value)} placeholder="Ex: R$ 500.000,00" />
              </div>
              <div>
                <label className={labelClass()}>Condições de pagamento</label>
                <input className={inputClass()} value={condicoes} onChange={(e) => setCondicoes(e.target.value)} placeholder="Ex: à vista no ato da assinatura" />
              </div>
            </div>
          </div>

          {/* Campos específicos por tipo */}
          {tipo === 'compra_venda_alienacao_fiduciaria' && (
            <div>
              <SectionTitle>Alienação Fiduciária</SectionTitle>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="md:col-span-3">
                  <label className={labelClass()}>Credor Fiduciário</label>
                  <input className={inputClass()} value={campos.credorFiduciario ?? ''} onChange={(e) => updateCampo('credorFiduciario', e.target.value)} placeholder="Nome do banco ou instituição financeira" />
                </div>
                <div>
          ...
          {tipo === 'compra_venda_hipoteca' && (
            <div>
              <SectionTitle>Hipoteca</SectionTitle>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="md:col-span-3">
                  <label className={labelClass()}>Credor Hipotecário</label>
                  <input className={inputClass()} value={campos.credor ?? ''} onChange={(e) => updateCampo('credor', e.target.value)} placeholder="Nome do credor" />
                </div>
                <div>
                  <label className={labelClass()}>Valor da Hipoteca</label>
                  <input className={inputClass()} value={campos.valorHipoteca ?? ''} onChange={(e) => updateCampo('valorHipoteca', e.target.value)} placeholder="Ex: R$ 200.000,00" />
                </div>
                <div>
                  <label className={labelClass()}>Prazo (meses)</label>
                  <input className={inputClass()} value={campos.prazoMeses ?? ''} onChange={(e) => updateCampo('prazoMeses', e.target.value)} placeholder="Ex: 120" />
                </div>
              </div>
            </div>
          )}
          ...
          {tipo === 'doacao' && (
            <div>
              <SectionTitle>Encargos da Doação</SectionTitle>
              <div>
                <label className={labelClass()}>Encargos (deixe em branco se doação pura)</label>
                <textarea className={`${inputClass()} resize-none`} rows={2} value={campos.encargos ?? ''} onChange={(e) => updateCampo('encargos', e.target.value)} placeholder="Descreva os encargos, se houver..." />
              </div>
            </div>
          )}
          ...
          {tipo === 'permuta' && (
            <div>
              <SectionTitle>Imóvel da Contraparte</SectionTitle>
              <div>
                <label className={labelClass()}>Descrição do imóvel a ser recebido em permuta</label>
                <textarea className={`${inputClass()} resize-none`} rows={3} value={campos.imovelPermutado ?? ''} onChange={(e) => updateCampo('imovelPermutado', e.target.value)} placeholder="Matrícula, descrição, endereço do imóvel permutado..." />
              </div>
            </div>
          )}
          ...
          <div className="pt-2">
            <button
              onClick={handleGerar}
              disabled={state.status === 'loading' || !tipo || !outorgante.nome || !outorgado.nome || !imovel.matricula}
            ...
            >
              {state.status === 'loading' ? (
                ...
                <><FileText className="w-4 h-4" />Gerar Minuta</>
              )}
            </button>
            <p className="text-xs text-slate-400 mt-2">
              A geração leva cerca de 15–30 segundos
            </p>
          </div>
        </>
      )}
